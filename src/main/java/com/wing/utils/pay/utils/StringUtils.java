...
public abstract class StringUtils {
    ...
    public static String[] split(String toSplit) {
        ...
        int offset = toSplit.indexOf(StringPool.COMMA);
        if (offset < 0) {
            return null;
        }
        String beforeDelimiter = toSplit.substring(0, offset);
        String afterDelimiter = toSplit.substring(offset + StringPool.COMMA.length());
    ...
    public static String filePathJoin(String[] paths) {
        ...
        int i = 0;
        for (int j = paths != null ? paths.length : 0; i < j; i++) {
            String path = paths[i];
            if (path == null)
                path = "";
            if (i > 0 && (path.startsWith(FOLDER_SEPARATOR) || path.startsWith(WINDOWS_FOLDER_SEPARATOR)))
                path = path.substring(1);
            stringbuffer.append(path);
            if (i + 1 < j && !path.endsWith(FOLDER_SEPARATOR) && !path.endsWith(WINDOWS_FOLDER_SEPARATOR))
